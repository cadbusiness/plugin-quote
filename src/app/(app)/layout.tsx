...
import { countUnreadProductUpdates } from "@/lib/updates/load";
  ...
  return (
    <div className="flex h-dvh overflow-hidden bg-slate-50">
      <Suspense fallback={<AppSidebar {...sidebarProps} snapshot={EMPTY_SIDEBAR_SNAPSHOT} />}>
        <SidebarWithSnapshot orgId={ctx.organization.id} userId={ctx.userId} {...sidebarProps} />
      </Suspense>
...
async function SidebarWithSnapshot({
  ...
  const [snapshot, unreadUpdates] = await Promise.all([
    ...
    countUnreadProductUpdates(supabase, userId),
  ]);
