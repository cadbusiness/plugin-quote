  ...
  return (
    <Window url="app.quotebuilder / produits">
      <div className="border-b border-slate-200 px-4 py-2.5">
        <p className="text-[11px] font-medium uppercase tracking-[0.14em] text-amber-700">Socle</p>
...
const AUTOPILOT_STEPS = [
  { label: "Confirmation prospect", when: "T+0", state: "Fait" },
  { label: "Notif commercial", when: "T+0", state: "Fait" },
  { label: "Rappel si non traité", when: "T+4 h", state: "Planifié" },
  { label: "Relance nurturing", when: "T+3 j", state: "En attente" },
];

export function AutopilotShot() {
  return (
    <Window url="app.quotebuilder / automations">
      <div className="border-b border-slate-200 px-4 py-2.5">
      ...
      <ul className="divide-y divide-slate-100 text-sm">
        {AUTOPILOT_STEPS.map((step) => (
          <li key={step.label} className="flex items-center justify-between gap-3 px-4 py-3">
            <div>
