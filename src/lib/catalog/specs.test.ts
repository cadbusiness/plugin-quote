...
{
  const prose = "Charge : 9999 kg/niveau. Hauteur 9 m. Profondeur 9 cm. Matériau bois. Délai 1 jour.";
  const ignored = mapWooProductSpecs({
    attributes: [{ name: "Charge", slug: "pa_charge", options: [""] }],
    meta_data: [],
    description: prose,
  } as { attributes: { name: string; slug: string; options: string[] }[]; meta_data: []; description: string });
...

{
  ...
  assert.equal(merged.charge?.value, "1200");
  assert.equal(merged.hauteur?.value, "250");
...
{
  assert.deepEqual(parseProductSpecs(JSON.stringify(parseProductSpecs(quickly))), quickly);
  ...
  const explicitEmptyUnit = parseProductSpecs({
...

{
  ...
  fields.set("specs", JSON.stringify({ materiau: { label: "Matériau", value: "Bois", unit: "" } }));
  const fromFields = readSpecsField(fields);
  assert.deepEqual(fromFields, {
    charge: { label: "Charge utile", value: "1000", unit: "kg/niveau" },
  });
  const cleared = new FormData();
  for (const key of ["charge", "hauteur", "profondeur", "materiau", "delai"] as const) {
    cleared.set(`spec_value_${key}`, "");
  }
  assert.deepEqual(readSpecsField(cleared), {});
}

console.log("catalog/specs ok");
