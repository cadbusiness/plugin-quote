...
Ce n’est pas forcément un CPQ Fortune 500. Chez beaucoup de PME, un [funnel de devis](/fonctionnalites/funnel) + [catalogue](/fonctionnalites/catalogue) + pipeline suffit largement avant d’acheter une usine à gaz.
...
Des articles francophones comme [Action Co sur les benchmarks business et outils CPQ](https://www.actionco.fr/strategie-commerciale-1218/les-benchmarks-du-business-2026-les-outils-cpq-dautomatisation-des-devis-43003) et des retours métier du type [automatiser les devis d’ingénierie avec configurateur](https://lumivi.fr/blog/automatiser-les-devis-dingenierie-avec-un-configurateur-et-lia-votre-cycle-de/) insistent sur le même fond : le goulot n’est souvent pas « savoir vendre », c’est **produire une offre juste, vite, traçable**.
...
Si vos demandes arrivent encore en texte libre, lisez [formulaire vs funnel](/blog/formulaire-contact-vs-funnel-devis-b2b) avant d’acheter quoi que ce soit.

### 4. Versions multiples

Devis_v3_final_VRAI.pdf. Tout le monde a vécu ça. Le directeur signe la v2. L’acheteur discute la v4. Le commercial croit que c’est la v3. Un espace prospect unique avec récap réduit ce chaos (voir [espace prospect](/fonctionnalites/espace-prospect)).
...
Le PDF part. Puis silence. Sans pipeline ni cadence, le devis meurt. Ce n’est pas un sujet « outil PDF », c’est un sujet process : [pourquoi les devis meurent sans relance](/blog/pourquoi-les-devis-meurent-sans-relance).

### 7. Données inexploitables

Vos Excel ne construisent pas facilement un historique : panier moyen par famille, taux de close par source, erreurs récurrentes. Sans données, pas de scoring sérieux (voir [score demande devis B2B](/blog/score-demande-devis-b2b)).
...
1. Le prospect configure via wizard / chat.
2. Le catalogue et les règles Si/Alors guident les options.
3. La demande arrive scorée dans le pipeline.
4. Confirmation, assignation, relances autopilote.
5. Le prospect garde un espace avec récap.

Vue d’ensemble : [comment ça marche](/comment-ca-marche). Tarifs indicatifs : Free 0 € ; Starter / Pro / Agency en annuel autour de 39 / 79 / 159 € par mois (mensuel plus haut). Détail : [tarifs](/tarifs).

Si votre catalogue vit déjà dans Woo ou Shopify, la sync évite la double saisie : [sync catalogue](/blog/sync-catalogue-woocommerce-shopify-parcours-devis). Pour le site : [widget WordPress / JS](/blog/installer-widget-devis-wordpress-javascript).

## Erreurs, marge, cycle time : comment les mesurer chez vous

Avant de migrer, prenez 15 dossiers récents et notez :

1. Temps entre demande et premier PDF envoyé.
2. Nombre d’aller-retour avant offre « propre ».
3. Écart entre marge théorique et marge réelle.
4. Nombre de versions PDF par dossier.
5. Devis sans aucune relance après J+3.

Ces cinq chiffres valent mieux qu’un benchmark externe. Ensuite seulement, vous comparez à un parcours guidé.

Pour estimer le manque à gagner des devis non suivis : [calculateur coût devis non relancé](/outils/cout-devis-non-relance).
...
Une fois l’entrée propre, priorisez : [score demande](/blog/score-demande-devis-b2b) et [générateur de séquence de relances](/outils/generateur-sequence-relances).

## Cas métiers où le configurateur tire vraiment son épingle

### Rayonnage / stockage

Charge, travées, normes : le message libre casse tout. Landing dédiée : [funnel devis rayonnage](/secteurs/funnel-devis-rayonnage-stockage).

### Menuiserie sur mesure

Cotes, matériaux, pose, contraintes chantier. Voir [funnel menuiserie](/secteurs/funnel-devis-menuiserie-sur-mesure).
...
**« On perd la flexibilité. »**  
Non si vous gardez une voie exception. Vous perdez surtout la flexibilité de vous tromper sans le voir.

**« Nos commerciaux n’aimeront pas. »**  
Ceux qui aiment chasser aiment rarement re-saisir 40 lignes. Donnez-leur le temps gagné sur le chiffrage répétitif.

**« Ça va être long à mettre en place. »**  
Un parcours MVP tient souvent en quelques jours si le catalogue est déjà un peu clair. Le vrai délai, c’est l’alignement prix internes.

**« On a déjà un ERP. »**  
L’ERP gère souvent la commande. Le parcours gère la **demande**. Ce n’est pas le même moment du cycle.
...
1. Prenez vos 10 derniers devis gagnés. Listez les champs vraiment utilisés.
2. Gardez seulement ceux qui changent le prix, le délai ou la faisabilité.
3. Créez un parcours de 8 écrans max.
4. Ajoutez 5 règles Si/Alors (les plus douloureuses en erreur humaine).
5. Branchez confirmation auto + owner.
6. Sur les 10 prochains dossiers, notez le temps gagné et les questions encore posées au téléphone.
...
Pour un secteur très configurable voisin : [rayonnage & stockage](/secteurs/funnel-devis-rayonnage-stockage) et [menuiserie sur mesure](/secteurs/funnel-devis-menuiserie-sur-mesure).
...
Non, et ce n’est pas le but. Il structure l’entrée, le pipeline et les relances. Les cas extrêmes restent hybrides. Voir [fonctionnalités](/fonctionnalites).
...
Si vous voulez tester la qualité d’un brief avant de chiffrer : [score brief devis](/outils/score-brief-devis). Si vous voulez voir le produit : [quotebuilder.co](/).
