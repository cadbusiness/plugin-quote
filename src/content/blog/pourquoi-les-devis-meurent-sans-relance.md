...
[ZoomInfo](https://pipeline.zoominfo.com/sales/sales-follow-up-statistics) insiste sur la persistance : le contact utile n’est souvent pas le premier interlocuteur, et le cycle B2B mélange plusieurs décideurs. Un devis « mort » est parfois un devis qui n’a jamais atteint la bonne personne. Relancer le même PDF au même e-mail cinq fois n’est pas de la persistance. Relancer avec un objet différent, un angle différent, et une porte de sortie claire, si.
...
**Personne ne mesure le manque à gagner.** Un devis non relancé n’apparaît pas dans le CA. Il n’apparaît pas non plus dans les stats Ads. Il disparaît. Le [calculateur de coût d’un devis non relancé](/outils/cout-devis-non-relance) rend ce trou visible : volume × panier × écart de taux. Sur 40 devis à 3 500 €, passer de 12 % à 22 % de closing, ce n’est pas un slogan. C’est un écart annuel que la plupart des directions commerciales n’ont jamais écrit.
...
Un enchaînement tenable ressemble à ceci — c’est aussi ce que seed l’[autopilote QuoteBuilder](/fonctionnalites/autopilote) :

1. **T+0 — Confirmation.** Le prospect reçoit la preuve que le dossier existe. Pièce jointe ou lien vers l’[espace prospect](/fonctionnalites/espace-prospect). Pas « nous avons bien reçu votre e-mail » : « voici ce que nous avons compris, voici comment compléter ».
...
5. **T+7 j — Nurturing.** Un angle métier, pas une relance vide. Sur le [rayonnage](/secteurs/funnel-devis-rayonnage-stockage), c’est souvent la charge ou le délai qui fait basculer, pas le premier prix catalogue.
...
Relancer un formulaire « bonjour, on voudrait un devis » produit des e-mails mous. Relancer un dossier — travées, charge, surface, budget, photos — produit des e-mails qui avancent. C’est toute la différence entre un [formulaire de contact et un funnel de devis](/blog/formulaire-contact-vs-funnel-devis-b2b).
...
Ensuite, l’ordre utile est toujours le même : offre → funnel → dossier → autopilote. C’est la chaîne de la [démo produit](/comment-ca-marche). Inverser — installer des relances sur un formulaire vague — reproduit le problème avec plus d’e-mails.
...
« On n’a pas le temps de rédiger six e-mails. » C’est précisément le sujet. On les rédige une fois. On les paramètre. On les arrête par branche quand le statut change. Le [générateur](/outils/generateur-sequence-relances) donne un premier jet. Le canvas QuoteBuilder tient la suite.
