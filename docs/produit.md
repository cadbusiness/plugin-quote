...
- **Sync WooCommerce (import + sync)**, Livré. `/integrations` : produits, descriptions, photos, prix, déclinaisons ; sync manuelle, planifiée et webhook. Fiche technique `products.specs` (charge, hauteur, profondeur, matériau, délai) depuis les attributs `pa_*` — `docs/woo-product-specs.md`. Rôles photo / plan / usage / notice, et mode d’emploi (`products.sheet`) — `docs/product-media.md`.
