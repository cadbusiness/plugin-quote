...
Le payload public du configurateur expose `products[].specs` comme une liste `{ key, label, value, unit? }`, issue d’abord de `products.options`, puis des clés encore présentes seulement dans la colonne. La colonne `products.specs` elle-même suit l’objet ci-dessus.
